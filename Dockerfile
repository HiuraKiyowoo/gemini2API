...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    go build -trimpath -ldflags="-s -w" -o /out/gemini2api .
...
COPY --from=backend-builder /out/gemini2api /usr/local/bin/gemini2api
COPY --from=frontend-builder /src/frontend/dist ./frontend/dist

RUN mkdir -p /app/data /app/logs /ms-playwright \
    && /usr/local/bin/gemini2api --install-browsers
