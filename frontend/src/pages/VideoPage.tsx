...
export default function VideoPage() {
  ...
  const [model, setModel] = useState("gemini-2.5-flash-video")
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setModel(current => chooseDefaultModel(options, current, "gemini-2.5-flash-video"))
      } catch {
  ...
  const handleDownload = async (url: string, idx: number) => {
    try {
      ...
      a.href = blobUrl
      a.download = `gemini_video_${Date.now()}_${idx}.mp4`
      a.click()
